...
import WeatherAndSearch from '../WeatherAndSearch/WeatherAndSearch'
import './Header.scss'

const StatusHeader = ['beforeAccess', 'Accessing', 'afterAccess']

function Header() {
  const currentUser = useSelector((state) => state.user.currentUser)
  const isMovingToAccessPage = useSelector(getRequestMovingToAccessPage)
  const isAcceptAccess = useSelector((state) => state.user.acceptAccess)
  const [accountTable, setAccountTable] = useState(false)
  const [header, setHeader] = useState(StatusHeader[0])
  const [energy, setEnergy] = useState(currentUser ? currentUser.energy : 0)
  ...
  const handleDisableMenu = () => {
    const status = user.current.getAttribute('class').includes('active')
    status && user.current.classList.remove('active')
  }

  useEffect(() => {
    if (isAcceptAccess) {
      setHeader(StatusHeader[2])
      openMenu.current.classList.remove('hiding')
    } else {
      if (isMovingToAccessPage) {
        setHeader(StatusHeader[1])
        openMenu.current.classList.add('hiding')
        user.current.classList.remove('active')
      } else {
        setHeader(StatusHeader[0])
        openMenu.current.classList.remove('hiding')
      }
    }
  }, [isAcceptAccess, isMovingToAccessPage])
  ...
  // } else if (breakpoint >= MAX_MOBILE && breakpoint < MAX_TABLET) {
  ...
  return (
    <div id="header">
      {(breakpoint > MAX_MOBILE || header !== StatusHeader[2]) && (
        <OfficialLogo />
      )}
      {header === StatusHeader[2] && <WeatherAndSearch />}

      <ul className="user" ref={user}>
        {header === StatusHeader[0] && (
          <>
            {breakpoint > MAX_MOBILE ? (
              <>
                <Link to="/sign-in">
                  <li>
                    <button className="user-item">Sign in</button>
                  </li>
                </Link>
                <p>|</p>
                <Link to="/sign-up">
                  <li>
                    <button className="user-item">Sign up</button>
                  </li>
                </Link>
              </>
            ) : (
              <div className="user-mobile-wrap">
                <div className="nav-close-wrap" onClick={handleClickIconClose}>
                ...
                <div className="user-mobile">
                  <div className="user-mobile-top-item">
                    <div className="user-mobile-top-wrap">
                      <h3>Need help?</h3>
                      <div className="user-mobile-top-help">
                        <Link to="/contact">
                          <p>Contact us</p>
                        </Link>
                        <Link to="/faq">
                  ...
                  <div className="user-mobile-top-item">
                    <div className="user-mobile-top-wrap">
                      <h3>Follow us</h3>
                      <div className="user-mobile-follow-list">
                        <a
                ...
                <div className="user-mobile-bottom">
                  ...
                  <div className="user-mobile-bottom-copyright">
                    <div className="user-mobile-country">
                      <svg
                        width="12"
                        height="13"
                        viewBox="0 0 12 13"
                    ...
                    <span className="user-mobile-copyright">
                      © 2022 HPS Inc.
                    </span>
                  </div>
                </div>
          ...
          <>
            ...
            {breakpoint > MAX_MOBILE ? (
                ...
                <div className="user-account" onClick={handleClickAccount}>
                  <img
                    src={
                      ...
                      require('../../common/user_avatar_default.png')
                    }
                    alt=""
              ...
              <div className="user-mobile-wrap">
                ...
                <div className="user-wrap">
                  ...
                  <div className="header-account">
                    <ul className="header-account-list">
                      ...
                      <Link
                        to="/"
                        className="header-account-item icon-account"
                        onClick={handleDisableMenu}
                      >
                        <i className="bi bi-gem"></i>
                        Upgrade to Premium
                ...
                <div className="user-mobile-bottom">
                  <div
                    className="user-mobile-top-item"
                    onClick={handleDisableMenu}
                  >
                    <div className="user-mobile-top-wrap">
                      <OfficialLogo />
      ...
      <i
        ref={openMenu}
        className="bi bi-list nav-open-icon"
