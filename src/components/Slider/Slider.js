...
import imageSlider from '../../resources/images/imageSlider'
import './Slider.scss'
      ...
      <div className="slider-container">
        {imageSlider.map((photo) => (
          <div key={photo.id}>
            ...
            <p
              className={
                indexDisplay === photo.id ? 'slider-page active' : 'slider-page'
              }
        ...
        <ul className="slider-frame">
          {imageSlider.map((photo) => (
            <li
              key={photo.id}
              className={
                photo.id === indexDisplay ? 'slider-btn active' : 'slider-btn'
              }
              onClick={() => {
                CurrentSlide(photo.id)
              }}
            ></li>
          ))}
        </ul>
        <div
          className="slider-change-left"
          onClick={() => {
            PlusSlide(-1)
          }}
        >
